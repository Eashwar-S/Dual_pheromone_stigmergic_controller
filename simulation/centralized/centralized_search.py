...
from common.visualization import (coverage_to_image, region_colors, 
                                                   draw_voronoi_borders, save_targets_over_time_plot, plot_visit_counts)
...
def sim_step(robots, covered, targets, found_targets, W, H, global_step, fail_at_step, 
    ...
    for r in robots:
        if not r.failed and r.idx != len(r.path) - 1:
            x, y = r.pos
            # visit_counts[y, x] += 1
            # mark_visited_counts(visit_counts, x, y, ROBOT_RADIUS)
    ...
    return global_step, failure_triggered, failure_reallocated, done
...
def update(frame, robots, covered, targets, found_targets, W, H, global_step, fail_at_step,
    ...
    for _ in range(steps_per_frame):
        global_step, failure_triggered, failure_reallocated, done = sim_step(
