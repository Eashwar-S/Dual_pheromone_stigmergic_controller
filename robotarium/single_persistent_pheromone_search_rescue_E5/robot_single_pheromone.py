...
RECENT_VISIT_CONCENTRATION_THRESHOLD = 0.35


@dataclass
class Robot:
    """Search robot using one evaporative field for repulsion and attraction."""
    ...
    def choose_move_search(
        ...
        """Prefer low concentration and avoid recently saturated pheromone."""
        ...
        for dx, dy in actions:
            ...
            pheromone_patch = pheromone[y_min:y_max, x_min:x_max]
            pheromone_sum = float(np.sum(pheromone_patch))
            candidates.append({
                "move": (nx, ny),
                "waypoint": (waypoint_x, waypoint_y),
                "uncovered": int(np.count_nonzero(~local_patch)),
                "pheromone": pheromone_sum,
                "concentration": pheromone_sum / max(1, pheromone_patch.size),
                "recently_visited": (
                    pheromone_sum / max(1, pheromone_patch.size)
                    >= RECENT_VISIT_CONCENTRATION_THRESHOLD
                ),
        ...
        candidates.sort(
            key=lambda candidate: (
                candidate["recently_visited"],
                -candidate["uncovered"],
                round(candidate["concentration"], 5),
    ...
    def record_waypoint(
        ...
        """Update motion memory after selecting a Robotarium waypoint."""
