...
from robot_random_memory import Robot
...
def run(show_figure: bool = True, max_steps: int = MAX_STEPS,
    ...
    # Each robot owns its map. These arrays are used for sensing and movement
    # selection but are never combined or shared between robot policies.
    local_maps = [robot.local_covered for robot in robots]
    ...
    found_plot = add_target_plot(r, targets, found_targets, "Memory Random Walk Robotarium", 0, show_figure)
    ...
    while len(found_targets) < len(targets) and not should_stop_simulation(step, max_steps, start_wall_time):
        ...
        refresh_found_plot(r, found_plot, found_targets, "Memory Random Walk Robotarium", step, len(targets))
    ...
    print(
        f"Memory random walk Robotarium completed: found {len(found_targets)}/{len(targets)} "
    ...
    return {
        ...
        "local_covered_cells": [
            int(np.count_nonzero(robot.local_covered))
            for robot in robots
        ],
