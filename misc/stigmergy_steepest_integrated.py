...
def spiral_generator():
    """Yields (dx, dy) for a square spiral pattern."""
    step_length = 1
...
class Robot:
    ...
    # Store the location of the target we are currently advertising
    ...
    def choose_move_search(self, pher_rep: np.ndarray, pher_attr: np.ndarray) -> Tuple[int, int]:
        ...
        for (nx, ny) in candidates:
            ...
            # Avoid areas with high repulsive pink pheromone
    ...
    # ---------------------------------------------------------
    # 2. FOLLOW MODE (Gradient Ascent - Seek MAX)
    # ---------------------------------------------------------
    def choose_move_follow(self, pher_attr: np.ndarray) -> Tuple[int, int]:
        ...
        # Filter: Must see SOME blue pheromone to follow it
        valid_candidates = []
        for (nx, ny) in candidates:
            val = pher_attr[ny, nx]
            if val > 1e-6:
                valid_candidates.append(((nx, ny), val))
        
        if not valid_candidates:
            self.mode = "SEARCH"
            return (self.x, self.y)
            
        # FUNDAMENTAL CHANGE: Seek MAX value (Gradient Ascent)
        # Because we paint the target bright and the edge dim.
        max_val = max(bc[1] for bc in valid_candidates)
        best_ones = [pos for pos, val in valid_candidates if val == max_val]
        
        return rng.choice(best_ones)

    # ---------------------------------------------------------
    # 3. DEPOSITION (Distance-Based Signal)
    # ---------------------------------------------------------
    def deposit_distance_signal(self, pher: np.ndarray, r: int = 5):
        """
        Deposits a non-cumulative signal based on distance to the target.
        Target = 10.0 (Brightest)
        Far = Lower (Dimmer)
        """
        ...
        # Calculate current distance from target to encode in pheromone
        dist_to_target = np.sqrt((x0 - tx)**2 + (y0 - ty)**2)
        
        # Signal Strength: High at target, decays with distance
        # 10.0 at dist=0, ~1.0 at dist=90
        signal_strength = 10.0 / (1.0 + 0.1 * dist_to_target)
        ...
        for dy in range(-r, r + 1):
            for dx in range(-r, r + 1):
                if abs(dx) + abs(dy) <= r:
                    cx, cy = x0 + dx, y0 + dy
                    if 0 <= cx < W and 0 <= cy < H:
                        # USE MAX, NOT +=. Prevents accumulation artifacts.
                        pher[cy, cx] = np.maximum(pher[cy, cx], signal_strength)

    def deposit_repulsive(self, pher: np.ndarray, amount, r):
        # Repulsive is still additive/cumulative as it marks "visited"
        H, W = pher.shape
        x0, y0 = int(self.x), int(self.y)
        for dy in range(-r, r + 1):
            for dx in range(-r, r + 1):
    ...
    def step(self, pher_rep: np.ndarray, pher_attr: np.ndarray, targets: Set[Tuple[int,int]]):
        ...
        if self.mode == "ADVERTISE":
            # Spiral Move
            if self.spiral_iter is None: self.spiral_iter = spiral_generator()
            try:
            ...
            # Deposit Gradient Signal (Non-Cumulative)
            self.deposit_distance_signal(pher_attr, r=5)
        
        elif self.mode == "SEARCH":
            # Check for Blue Trail
            candidates = []
            if self.y-1>=0: candidates.append((self.x, self.y-1))
            if self.y+1<H:  candidates.append((self.x, self.y+1))
            if self.x-1>=0: candidates.append((self.x-1, self.y))
...
def combined_pheromone_to_rgba(p_rep: np.ndarray, p_attr: np.ndarray) -> np.ndarray:
    ...
    # Blue (Attractive)
    # Fixed scale for consistency with new "10.0" max signal
...
def sim_step():
    ...
    # Evaporation (Less critical for Blue now, but good for cleanup)
