...
def _run_task(task: Tuple[int, int, int, int, int, str, int, List[Tuple[int, int]], int]) -> Tuple[int, Dict[str, object]]:
    order, grid_size, n_robots, n_targets, n_failures, failure_time_mode, exp_idx, failure_schedule, run_seed = task

    print(
        f"Running Centralized: grid={grid_size}, robots={n_robots}, "
        f"targets={n_targets}, failures={n_failures}, mode={failure_time_mode}, exp={exp_idx}",
        flush=True,
    )
...
def _build_tasks() -> List[Tuple[int, int, int, int, int, str, int, List[Tuple[int, int]], int]]:
    tasks = []
    order = 0
    schedule_seed = 42
