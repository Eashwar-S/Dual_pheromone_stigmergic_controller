...
def update_robotarium(r, x_pose, grid_goals, helpers):
    ...
    dxu[0, :] = np.clip(
        dxu[0, :],
        -r.MAX_LINEAR_VELOCITY,
        r.MAX_LINEAR_VELOCITY,
    )
    dxu[1, :] = np.clip(
        dxu[1, :],
        -r.MAX_ANGULAR_VELOCITY,
        r.MAX_ANGULAR_VELOCITY,
    )
...
def deposit_distance_signal(
    ...
    """Deposit a target-directed attractive signal along the advertiser path."""
    ...
    for dy in range(-robot_radius, robot_radius + 1):
        for dx in range(-robot_radius, robot_radius + 1):
            ...
            cx, cy = x + dx, y + dy
            if 0 <= cx < pheromone.shape[1] and 0 <= cy < pheromone.shape[0]:
                pheromone[cy, cx] = max(pheromone[cy, cx], signal)
...
def attractive_pheromone_to_rgba(
    pheromone: np.ndarray,
) -> np.ndarray:
    vmax = max(float(np.max(pheromone)), PHER_MIN)
    normalized = np.clip(pheromone / vmax, 0.0, 1.0)
    rgba = np.zeros((*pheromone.shape, 4), dtype=float)
    rgba[..., 0] = 1.00
    rgba[..., 1] = 0.12
    rgba[..., 2] = 0.55
    rgba[..., 3] = normalized * 0.82
    return rgba
...
def add_pheromone_overlays(
    ...
    if not show_figure:
        return None, None
    ax = get_robotarium_axes(r)
    repulsive_plot = ax.imshow(
        pheromone_to_rgba(repulsive_pheromone),
        origin="lower",
        extent=ACTIVE_WORLD_BOUNDS,
        interpolation="nearest",
        zorder=-5,
    )
    attractive_plot = ax.imshow(
        attractive_pheromone_to_rgba(attractive_pheromone),
        origin="lower",
        extent=ACTIVE_WORLD_BOUNDS,
        interpolation="nearest",
        zorder=-4,
    )
    return repulsive_plot, attractive_plot


def refresh_pheromone_overlays(
    plots,
    repulsive_pheromone: np.ndarray,
    attractive_pheromone: np.ndarray,
) -> None:
    repulsive_plot, attractive_plot = plots
    if repulsive_plot is not None:
        repulsive_plot.set_data(
            pheromone_to_rgba(repulsive_pheromone)
        )
    if attractive_plot is not None:
        attractive_plot.set_data(
            attractive_pheromone_to_rgba(attractive_pheromone)
        )
...
def update_dual_title(
    ...
    if not show_figure:
        return
    statuses = [
        f"R{robot_id}: {modes[robot_id]}"
        + (" \N{CHECK MARK}" if robot_id in found_robot_ids else "")
        for robot_id in ROBOT_IDS
    ]
    get_robotarium_axes(r).set_title(
        f"Dual-Pheromone Search and Rescue | Step {step} | "
        f"Found: {len(found_robot_ids)}/{N_ROBOTS} | "
        + " | ".join(statuses)
    )
...
def print_dual_position_report(
    step: int,
    grid_positions: Sequence[Tuple[int, int]],
    x_pose: np.ndarray,
    modes: Sequence[str],
    ...
    parts = []
    for robot_id in ROBOT_IDS:
        found = " found" if robot_id in found_robot_ids else ""
        parts.append(
            f"R{robot_id} grid={grid_positions[robot_id]} "
            f"world=({x_pose[0, robot_id]:.3f}, "
            f"{x_pose[1, robot_id]:.3f}) "
            f"mode={modes[robot_id]}{found}"
        )
    print(
        f"Step {step}: "
        + "; ".join(parts)
        + f"; repulsive_max={float(np.max(repulsive_pheromone)):.4f}"
        + f"; attractive_max={float(np.max(attractive_pheromone)):.4f}"
    )
