...
def bfs_path_in_mask(mask: np.ndarray,
                      start: Tuple[int, int],
    ...
    q = deque([start])
    prev = {start: None}                    # dictionary for backtracking
    nbrs = [(1,0), (-1,0), (0,1), (0,-1)]   # actions to visit neighbours
...
def connect_inside_mask(path: List[Tuple[int, int]],
                        mask: np.ndarray,
                        to_xy: Tuple[int, int]) -> bool:
    ...
    a = path[-1]
    bfs = bfs_path_in_mask(mask, a, to_xy)
...
def generate_sweep_path_for_region(mask: np.ndarray, robot_radius: int) -> List[Tuple[int, int]]:
    """
    Generate boustrophedon-style sweeping path for a region mask.
    """
    H, W = mask.shape
    path: List[Tuple[int, int]] = []
    step_size = max(1, 2 * robot_radius - 1) # stride distance between sweeps with minimum intersection

    # Find all the x and y coordinates of the region to sweep
    ys, xs = np.where(mask)
    if xs.size == 0:
        return path
    ...
    # Choose bounding-box corner. Snap to nearest mask cell if corner not inside.
    ...
    while y0 <= y_max:
        ...
        for (a, b) in segs:
            ...
            ok = connect_inside_mask(path, mask, (run[0], sweep_y))
            if not ok:
        ...
        if next_y0 <= y_max:
            ...
            if next_sweep_y is not None:
                ...
                if xs_next:
                    ...
                    connect_inside_mask(path, mask, (x_next, next_sweep_y))
