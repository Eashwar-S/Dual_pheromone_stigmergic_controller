...
@dataclass
class Robot:
    """Multi-mode robot with SEARCH/FOLLOW/ADVERTISE modes using spiral pattern."""
    id: int
    x: int
    y: int
    ...
    def deposit_distance_signal(self, pher: np.ndarray, r: int = 5):
        """Deposit exponential distance-based signal for advertising target."""
        if self.advertising_target is None:
            return
