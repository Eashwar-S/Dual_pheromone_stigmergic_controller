...
from stigmergy_search_efficient_experiment import run_simulation
...
def _run_task(task: Tuple[int, int, int, int, int, str, int, int, List[Tuple[int, int]], int, int]) -> Tuple[int, Dict[str, object]]:
    ...
    print(
        f"Running Efficient Stigmergy: grid={grid_size}, robots={n_robots}, targets={n_targets}, "
...
def _write_results(rows: List[Dict[str, object]]) -> Path:
    ...
    scalar_rows, timeseries_rows = split_scalar_and_timeseries(rows, "stigmergy_search_efficient")
